...
- 数据库属性默认值、必填与唯一值约束统一进入核心模型、SQLite 事务、批量编辑、模板和 CSV 导入。
- SQLite Schema v16：数据库记录属性/正文变更历史、200 条有界留存、差异查看与约束安全的单次恢复。
- SQLite Schema v17：数据库记录评论、属性级讨论锚点、解决/重新打开与未解决筛选。
- SQLite Schema v18：日期属性记录提醒、到期索引、完成/重新打开与记录级提醒管理。
- 双向 Relation 支持显式反向属性绑定、SQLite 单事务增删同步、历史留痕和浏览器回退一致性。
- 属性删除会追踪跨数据库 Rollup、同库 Formula 和反向 Relation 依赖，阻止破坏性删除并安全解除反向绑定。
- 工程代码文件 1000 行硬门禁；数据库、应用弹层、数据库视图和样式已按职责拆分。
- SQLite 访问层开始规范化为 `sql/` 查询清单、`repositories/` 预编译仓储和领域服务三层，并由 SQL 边界门禁禁止业务模块新增内嵌查询。

### 当前验证结果

- TypeScript：通过。
- Vitest：27 个测试文件、155 个测试通过。
...
- 当前视图属性显隐、三档行高、80–600px 列宽拖拽与刷新持久化：通过。
- 视图级属性拖拽排序、冻结首列、类型安全列底计算与 10,000 行虚拟化：通过。
- 数据库即时搜索、视图级快速筛选、分组折叠刷新恢复和窄宽工具栏 Chromium 验收：通过。
- 10,000 条记录视图级手动顺序、表格拖拽、看板跨组移动与 SQLite 恢复：通过。
- Schema v15 记录复制、千条批量软删除、回收站恢复与永久删除事务：通过。
- 属性默认值、必填/唯一值冲突、批量/导入回滚和恢复校验：通过；真实 Chromium 属性面板交互与视觉验收通过。
- Schema v16 属性/正文历史、自动化写入留痕、单次恢复和 200 条上限：通过；记录详情历史面板 Chromium 验收通过。
- Schema v17 整条记录/属性评论、未解决筛选、解决与删除：通过；真实 Chromium 创建、筛选和清理闭环通过。
- Schema v18 日期提醒、全局到期查询、完成/重新打开和删除：通过；真实 Chromium 到期状态与清理闭环通过。
- 双向 Relation 目标校验、添加/移除反向同步、无效反向属性拒绝和事务回滚边界：通过。
- 属性依赖阻断、跨库 Rollup 目标追踪、Formula 引用识别和反向 Relation 安全解绑：通过。
- 全仓代码文件行数门禁：通过，最大文件不超过 1000 行。
- SQL 边界门禁：通过；工作区页面、数据库快照读取、collaboration 与 platform 全领域查询已迁移到独立查询清单与预编译仓储。
- records 的 Schema、属性、数据库元数据和安全删除事务已完成仓储化，业务模块内嵌查询由 89 处降至 66 处。
- records 的单元格写入、唯一值校验、双向 Relation 和记录历史已完成仓储化，内嵌查询继续降至 53 处。
- records 的属性级评论、未解决线程、日期提醒和到期查询已完成仓储化，内嵌查询继续降至 41 处。
- records 的创建、复制、回收站、约束安全恢复、永久删除和正文写入已完成仓储化，内嵌查询继续降至 26 处。
...
8. 视图级属性显示、表格列宽拖拽、三档行高与现有视图 JSON 持久化已交付。
9. 视图级属性排序、冻结首列和按属性类型约束的列底计算已交付。
10. 数据库即时搜索、快速筛选和分组折叠状态持久化已交付。
11. 数据库记录手动排序、分组内拖放和视图级记录顺序持久化已交付。
12. 数据库记录复制、批量删除和可恢复记录回收站已交付。
13. 属性默认值、必填约束和唯一性校验已交付，桌面与浏览器回退路径行为一致。
14. 数据库记录属性/正文变更历史、版本差异与单次恢复已交付。
15. 数据库记录评论、属性级讨论与未解决线程筛选已交付。
16. 日期属性提醒、到期通知查询与记录级提醒管理已交付。
17. 双向 Relation、反向关联属性与桌面/浏览器一致性维护已交付。
18. 属性依赖追踪、安全删除与反向 Relation 解绑已交付。
19. 下一步实现 Relation 显示数量限制、关联搜索与记录预览。
