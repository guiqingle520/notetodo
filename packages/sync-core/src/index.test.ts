...
describe('sync core', () => {
  ...
  it('batches rapid updates into one persistence write', async () => {
    ...
    const persistedUpdate = writes[0]
    if (!persistedUpdate) throw new Error('The update batch was not persisted.')
    Y.applyUpdate(restored, persistedUpdate)
  ...
  it('authenticates, flushes offline updates and reconnects with backoff', async () => {
    ...
    if (!firstSocket) throw new Error('The initial collaboration socket was not created.')
    firstSocket.open()
    const initialAuth = firstSocket.sent[0]
    if (!initialAuth) throw new Error('The initial authentication frame was not sent.')
    expect(JSON.parse(initialAuth)).toEqual(expect.objectContaining({ token: 'secret' }))
    firstSocket.message({ type: 'auth-ok', clientId: 'client-1' })
    firstSocket.message({ type: 'seed-required' })
    expect(seedRequired).toHaveBeenCalledOnce()
    expect(firstSocket.sent.some((item) => item.includes('offline-update'))).toBe(true)

    firstSocket.serverClose()
    await vi.advanceTimersByTimeAsync(500)
    expect(sockets).toHaveLength(2)
    const secondSocket = sockets[1]
    if (!secondSocket) throw new Error('The reconnect socket was not created.')
    secondSocket.open()
    await vi.advanceTimersByTimeAsync(0)
    expect(refreshToken).toHaveBeenCalledOnce()
    const refreshedAuth = secondSocket.sent[0]
    if (!refreshedAuth) throw new Error('The refreshed authentication frame was not sent.')
    expect(JSON.parse(refreshedAuth)).toEqual(expect.objectContaining({ token: 'refreshed-secret' }))
