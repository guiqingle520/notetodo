...
interface Window {
  notetodo?: {
    ...
    workspace: {
      ...
      upsertPage: (
        page: import('./domain').WorkspacePage,
      ) => Promise<import('./domain').WorkspacePage>
    ...
    history: {
      list: (pageId: string) => Promise<
        Array<{
          id: number
          pageId: string
          title: string
          reason: 'autosave' | 'restore'
          createdAt: string
    ...
    automations: {
      ...
      listRuns: (databaseId: string) => Promise<
        Array<{
          ...
          output: Array<{
            propertyId: string
            value: import('@notetodo/automation-core').AutomationValue
          }>
    ...
    collaboration: {
      getTicket: (pageId: string) => Promise<null | {
        endpoint: string
        token: string
        userId: string
        name: string
    ...
    imports: {
      pickAndInspect: () => Promise<null | {
        ...
        entries: Array<{
          path: string
          kind: 'page' | 'database' | 'asset' | 'sitemap' | 'unsupported'
          size: number
        }>
      ...
      listJobs: () => Promise<
        Array<{
          id: string
          sourceName: string
          status: 'converting' | 'committing' | 'completed' | 'failed' | 'cancelled'
    ...
    model: {
      ...
      testConnection: () => Promise<{ ok: true; latencyMs: number; endpoint: string }>
      streamChat: (
        request: {
        ...
        onEvent: (event: {
          type: 'text-delta' | 'usage' | 'done' | 'cancelled' | 'error'
          text?: string
          message?: string
          inputTokens?: number
          outputTokens?: number
