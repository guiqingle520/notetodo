...
import { ArrowRight, BookOpen, Calculator, Database, GripVertical, Link2, RotateCcw, Sigma, X } from 'lucide-react'
import { calculateColumn, validColumnCalculations, virtualWindow, type ColumnCalculation, type DatabaseProperty, type DatabaseRecord, type DatabaseRecordHistory, type DatabaseSchema, type DatabaseViewConfig, type PropertyValue } from '@notetodo/database-core'
...
export function RecordDetailPanel({ record, schema, relationTargets = {}, onClose, onUpdateCell, onUpdateContent, onListHistory, onRestoreHistory }: { record: DatabaseRecord; schema: DatabaseSchema; relationTargets?: RelationTargets; onClose: () => void; onUpdateCell: (recordId: string, propertyId: string, value: PropertyValue) => void; onUpdateContent: (recordId: string, content: string) => void; onListHistory?: () => Promise<DatabaseRecordHistory[]>; onRestoreHistory?: (historyId: string) => Promise<void> }) {
  ...
  const openHistory = async () => { persist(); setHistoryOpen(true); if (onListHistory) setHistory(await onListHistory()) }
  const persist = () => {
  ...
  return <div className="record-detail-backdrop" onMouseDown={close}>
    <section className="record-detail-panel" role="dialog" aria-modal="true" aria-label={`记录详情：${title || '无标题'}`} onMouseDown={(event) => event.stopPropagation()}>
      <header><span><BookOpen size={13} />{schema.name}<i>/</i>{record.id.slice(0, 8)}</span><div><em className={`is-${saveState}`}>{saveState === 'saving' ? '正在保存…' : '已保存到本地'}</em><button aria-label="记录历史" className={historyOpen ? 'is-active' : ''} onClick={() => historyOpen ? setHistoryOpen(false) : void openHistory()}><RotateCcw size={14} /></button><button aria-label="关闭记录详情" onClick={close}><X size={16} /></button></div></header>
