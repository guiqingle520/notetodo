...
function GenericCell({ record, recordLabel, property, relationTargets, updateCell, onOpenRecord }: { record: DatabaseRecord; recordLabel: string; property: DatabaseProperty; relationTargets: RelationTargets; updateCell: (recordId: string, propertyId: string, value: PropertyValue) => void; onOpenRecord?: (recordId: string) => void }) {
  const value = record.values[property.id]
  const label = `编辑${recordLabel}的${property.name}`
  if (property.type === 'select') return <select aria-label={label} className={`generic-select select-color-${property.options?.find((option) => option.id === value)?.color ?? 'empty'}`} value={String(value ?? '')} onChange={(event) => updateCell(record.id, property.id, event.target.value || null)}><option value="">未选择</option>{property.options?.map((option) => <option key={option.id} value={option.id}>{option.name}</option>)}</select>
  if (property.type === 'checkbox') return <label className="generic-check"><input aria-label={label} type="checkbox" checked={Boolean(value)} onChange={(event) => updateCell(record.id, property.id, event.target.checked)} /><span /></label>
  if (property.type === 'number') return <input aria-label={label} type="number" value={typeof value === 'number' ? value : ''} onChange={(event) => updateCell(record.id, property.id, event.target.value === '' ? null : Number(event.target.value))} />
  if (property.type === 'date') return <input aria-label={label} type="date" value={typeof value === 'string' ? value : ''} onChange={(event) => updateCell(record.id, property.id, event.target.value || null)} />
  if (property.type === 'multiSelect') return <input aria-label={label} value={Array.isArray(value) ? value.join(', ') : ''} placeholder="逗号分隔" onChange={(event) => updateCell(record.id, property.id, event.target.value.split(',').map((item) => item.trim()).filter(Boolean))} />
  if (property.type === 'relation') return <RelationValueEditor label={label} value={Array.isArray(value) ? value : []} target={property.relation?.databaseId ? relationTargets[property.relation.databaseId] : undefined} onChange={(next) => updateCell(record.id, property.id, next)} />
  if (property.type === 'formula' || property.type === 'rollup') return <output aria-label={`${recordLabel}的${property.name}`}>{Array.isArray(value) ? value.join('、') : value === null || value === undefined ? '—' : String(value)}</output>
  if (property.type === 'title') return <div className="generic-title-cell"><input aria-label={label} className="is-title" value={typeof value === 'string' ? value : ''} onChange={(event) => updateCell(record.id, property.id, event.target.value)} /><button aria-label={`打开 ${String(value || '无标题')}`} title="打开记录详情" onClick={() => onOpenRecord?.(record.id)}><BookOpen size={12} /></button></div>
  return <input aria-label={label} type={property.type === 'url' ? 'url' : 'text'} value={typeof value === 'string' ? value : ''} onChange={(event) => updateCell(record.id, property.id, event.target.value)} />
}

function RelationValueEditor({ label, value, target, onChange }: { label: string; value: string[]; target?: RelationTargets[string]; onChange: (value: string[]) => void }) {
  ...
  return <details className="relation-value-editor"><summary aria-label={label}>{value.length ? value.slice(0, 2).map((id) => names.get(id) ?? id).join('、') + (value.length > 2 ? ` +${value.length - 2}` : '') : '添加关联'}</summary><div>{target?.records.slice(0, 100).map((candidate) => <label key={candidate.id}><input type="checkbox" checked={value.includes(candidate.id)} onChange={() => onChange(value.includes(candidate.id) ? value.filter((id) => id !== candidate.id) : [...value, candidate.id].slice(0, 100))} /><span>{names.get(candidate.id)}</span></label>)}{!target?.records.length && <p>目标数据库还没有记录。</p>}</div></details>
}
...
function RecordPropertyField({ record, property, relationTargets, onUpdate }: { record: DatabaseRecord; property: DatabaseProperty; relationTargets: RelationTargets; onUpdate: (recordId: string, propertyId: string, value: PropertyValue) => void }) {
  ...
  else if (property.type === 'relation') field = <RelationValueEditor label={`编辑${property.name}`} value={Array.isArray(value) ? value : []} target={property.relation?.databaseId ? relationTargets[property.relation.databaseId] : undefined} onChange={(next) => onUpdate(record.id, property.id, next)} />
...
export function ListView({ records, updateCell }: { records: DatabaseRecord[]; updateCell: (recordId: string, propertyId: string, value: PropertyValue) => void }) {
  ...
  return <div className="database-list" role="region" aria-label="列表视图" tabIndex={0} onScroll={(event) => setScrollTop(event.currentTarget.scrollTop)}><div className="database-list-space" style={{ height: totalSize }}>{records.slice(start, end).map((record, offset) => <div className="database-list-row" key={record.id} style={{ transform: `translateY(${(start + offset) * LIST_ROW_HEIGHT}px)` }}><span className="list-index">{String(start + offset + 1).padStart(2, '0')}</span><input value={String(record.values['task-title'] ?? '')} onChange={(event) => updateCell(record.id, 'task-title', event.target.value)} /><StatusSelect record={record} updateCell={updateCell} /><span>{record.values['task-owner']}</span><time>{record.values['task-due']}</time></div>)}</div></div>
}
