...
export function DatabaseBlock({ pageId, initialSnapshot }: { pageId: string; initialSnapshot?: DatabaseSnapshot }) {
  ...
  const rulesDialogId = `${panelIdPrefix}-rules-dialog`
  const schemaDialogId = `${panelIdPrefix}-schema-dialog`
  const csvDialogId = `${panelIdPrefix}-csv-dialog`
  const trashDialogId = `${panelIdPrefix}-trash-dialog`
  const automationDialogId = `${panelIdPrefix}-automation-dialog`
    ...
    <section className="database-block">
      ...
      <div className="database-viewbar">
        ...
        <div className="database-tools" role="toolbar" aria-label="数据库工具">
          ...
          <button className={schemaOpen ? 'is-active' : ''} aria-haspopup="dialog" aria-expanded={schemaOpen} aria-controls={schemaDialogId} onClick={() => { setLayoutOpen(false); setSchemaOpen(true) }}><Settings2 size={13} />属性 · {snapshot.schema.properties.length}</button>
          ...
          <button className={activeView.config.filters?.length ? 'is-active' : ''} aria-haspopup="dialog" aria-expanded={rulesOpen === 'filters'} aria-controls={rulesDialogId} onClick={() => setRulesOpen('filters')}><Filter size={13} />筛选{activeView.config.filters?.length ? ` · ${activeView.config.filters.length}` : ''}</button>
          ...
          <button className={activeView.config.sorts?.length ? 'is-active' : ''} aria-haspopup="dialog" aria-expanded={rulesOpen === 'sorts'} aria-controls={rulesDialogId} onClick={() => setRulesOpen('sorts')}><ArrowUpDown size={13} />排序{activeView.config.sorts?.length ? ` · ${activeView.config.sorts.length}` : ''}</button>
          <button className={activeView.config.groupByPropertyId ? 'is-active' : ''} aria-haspopup="dialog" aria-expanded={rulesOpen === 'group'} aria-controls={rulesDialogId} onClick={() => setRulesOpen('group')}><Layers3 size={13} />分组</button>
          ...
          <button className={csvImportOpen ? 'is-active' : ''} aria-haspopup="dialog" aria-expanded={csvImportOpen} aria-controls={csvDialogId} onClick={() => setCsvImportOpen(true)}><FileUp size={13} />导入</button>
          <button className={recordTrashOpen ? 'is-active' : ''} aria-haspopup="dialog" aria-expanded={recordTrashOpen} aria-controls={trashDialogId} onClick={() => void openRecordTrash()}><Trash2 size={13} />回收站</button>
          ...
          <button className="database-automation" aria-haspopup="dialog" aria-expanded={automationOpen} aria-controls={automationDialogId} onClick={() => setAutomationOpen(true)}><Zap size={13} />自动化 · {automations.filter((rule) => rule.enabled).length}</button>
      ...
      {csvImportOpen && createPortal(<CsvImportPanel id={csvDialogId} schema={snapshot.schema} onClose={() => setCsvImportOpen(false)} onImport={importCsvRecords} />, document.body)}
      {recordTrashOpen && createPortal(<RecordTrashPanel id={trashDialogId} records={trashedRecords} onClose={() => setRecordTrashOpen(false)} onRestore={restoreTrashedRecords} onDeletePermanently={deleteTrashedRecords} />, document.body)}
      ...
      {automationOpen && <AutomationPanel id={automationDialogId} schema={snapshot.schema} rules={automations} runs={automationRuns} onClose={() => setAutomationOpen(false)} onSave={async (rule) => {
...
export function CsvImportPanel({ id, schema, onClose, onImport }: {
  id?: string; schema: DatabaseSchema; onClose: () => void; onImport: (rows: Array<Record<string, PropertyValue>>) => Promise<void>
}) {
  const dialogRef = useDialogFocus<HTMLElement>()
  ...
  return <div className="schema-panel-backdrop csv-import-backdrop" onMouseDown={onClose}><section id={id} ref={dialogRef} className="csv-import-panel" role="dialog" aria-modal="true" aria-label="导入 CSV" tabIndex={-1} onMouseDown={(event) => event.stopPropagation()}>
...
export function RecordTrashPanel({ id, records, onClose, onRestore, onDeletePermanently }: { id?: string; records: DatabaseTrashRecord[]; onClose: () => void; onRestore: (recordIds: string[]) => Promise<void>; onDeletePermanently: (recordIds: string[]) => Promise<void> }) {
  const dialogRef = useDialogFocus<HTMLElement>()
  ...
  return <div className="record-trash-backdrop" role="presentation" onMouseDown={onClose}><section id={id} ref={dialogRef} className="record-trash-panel" role="dialog" aria-modal="true" aria-label="数据库记录回收站" tabIndex={-1} onMouseDown={(event) => event.stopPropagation()}>
