...
module.exports = Object.freeze({
  ...
  automationSchema:
    'SELECT id, type FROM database_properties WHERE database_id=? ORDER BY position',
  ...
  automationRecordValues:
    'SELECT property_id, text_value, number_value, boolean_value, json_value FROM property_values WHERE record_id=?',
  automationProperty:
  ...
  failedAutomationRun: `SELECT id, automation_id, automation_name, database_id, record_id, trigger_property_id,
      rule_json, input_json, output_json, status, error_message, replay_of, created_at, completed_at
    FROM automation_runs WHERE id=? AND status='failed'`,
  ...
  pendingWebhook:
    "SELECT id FROM webhook_outbox WHERE endpoint_id=? AND event=? AND resource_key=? AND status='pending' ORDER BY created_at DESC LIMIT 1",
  updatePendingWebhook:
    'UPDATE webhook_outbox SET payload_json=?, next_attempt_at=?, created_at=? WHERE id=?',
  insertWebhook: `INSERT INTO webhook_outbox(id, endpoint_id, event, resource_key, payload_json, status, next_attempt_at, created_at)
    VALUES (?, ?, ?, ?, ?, 'pending', ?, ?)`,
  recoverWebhookLeases:
    "UPDATE webhook_outbox SET status='pending', lease_owner=NULL, lease_until=NULL WHERE status='leased' AND lease_until<=?",
  ...
  leaseWebhook:
    "UPDATE webhook_outbox SET status='leased', lease_owner=?, lease_until=? WHERE id=? AND status='pending'",
  ...
  ownedWebhook:
    "SELECT attempts FROM webhook_outbox WHERE id=? AND status='leased' AND lease_owner=?",
  ...
  insertApiToken: `INSERT INTO api_tokens(id, name, token_prefix, secret_hash, scopes_json, expires_at, created_at)
    VALUES (?, ?, ?, ?, ?, ?, ?)`,
  listApiTokens:
    'SELECT id, name, token_prefix, scopes_json, expires_at, revoked_at, last_used_at, created_at FROM api_tokens ORDER BY created_at DESC',
  apiTokenById:
  ...
  insertApiAudit: `INSERT INTO api_audit_log(request_id, token_id, method, path, status, duration_ms, created_at)
    VALUES (?, ?, ?, ?, ?, ?, ?)`,
  listApiAudit:
    'SELECT request_id, token_id, method, path, status, duration_ms, created_at FROM api_audit_log ORDER BY created_at DESC, id DESC LIMIT ?',
})
