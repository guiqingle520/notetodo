module.exports = Object.freeze({
  ...
  syncUpdates:
    'SELECT id, client_id, update_blob FROM sync_updates WHERE page_id = ? AND id > ? ORDER BY id',
  appendSyncUpdate:
    'INSERT INTO sync_updates(page_id, client_id, update_blob, created_at) VALUES (?, ?, ?, ?)',
  ...
  permissionsByPage:
    'SELECT subject_id AS subjectId, display_name AS displayName, role FROM page_permissions WHERE page_id = ? ORDER BY role DESC, display_name',
  removePermission:
    "DELETE FROM page_permissions WHERE page_id=? AND subject_id=? AND role<>'owner'",
  pageRole: 'SELECT role FROM page_permissions WHERE page_id = ? AND subject_id = ?',
  createComment:
    'INSERT INTO comments(id, page_id, author_id, author_name, body, anchor_json, resolved_at, created_at, mentions_json) VALUES (?, ?, ?, ?, ?, ?, NULL, ?, ?)',
  createNotification:
    'INSERT INTO notifications(id, recipient_id, page_id, comment_id, type, read_at, created_at) VALUES (?, ?, ?, ?, ?, NULL, ?)',
  commentsByPage:
    'SELECT id, author_name AS authorName, body, anchor_json AS anchor, mentions_json AS mentions, resolved_at AS resolvedAt, created_at AS createdAt FROM comments WHERE page_id = ? ORDER BY created_at DESC',
