...
module.exports = Object.freeze({
  ...
  `,
  listPages: `
    SELECT id, title, description, icon, parent_id, favorite, content,
  ...
  upsertPage: `
    INSERT INTO pages (
      id, title, description, icon, parent_id, favorite, content,
      updated_at, last_visited_at, archived_at
    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  ...
  searchPages: `
    SELECT p.id, p.title, p.description, p.icon, p.parent_id, p.favorite, p.content,
  ...
  recentPages: `
    ...
    LIMIT ?
  `,
  pageById: `SELECT id, title, description, icon, parent_id, favorite, content,
