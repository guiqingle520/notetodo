...
function registerWorkspaceIpc(database) {
  ...
  ipcMain.handle('database:update-view-config', (_event, databaseId, viewId, config) => {
    ...
    if ((config.filters !== undefined && !Array.isArray(config.filters)) || (config.sorts !== undefined && !Array.isArray(config.sorts)) || (config.visiblePropertyIds !== undefined && !Array.isArray(config.visiblePropertyIds)) || (config.propertyOrder !== undefined && !Array.isArray(config.propertyOrder))) throw new TypeError('Database view rules must be arrays.')
    ...
    if (serialized.length > 50_000 || (config.filters?.length ?? 0) > 20 || (config.sorts?.length ?? 0) > 10 || (config.visiblePropertyIds?.length ?? 0) > 50 || (config.propertyOrder?.length ?? 0) > 50 || Object.keys(config.propertyWidths ?? {}).length > 50 || Object.keys(config.calculations ?? {}).length > 50) throw new TypeError('Database view configuration is too large.')
