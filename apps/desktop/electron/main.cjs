...
function registerWorkspaceIpc(database) {
  ...
  ipcMain.handle('database:update-view-config', (_event, databaseId, viewId, config) => {
    ...
    if (serialized.length > 50_000 || (config.filters?.length ?? 0) > 20 || (config.quickFilters?.length ?? 0) > 5 || (config.sorts?.length ?? 0) > 10 || (config.visiblePropertyIds?.length ?? 0) > 50 || (config.propertyOrder?.length ?? 0) > 50 || (config.collapsedGroupKeys?.length ?? 0) > 100 || Object.keys(config.propertyWidths ?? {}).length > 50 || Object.keys(config.calculations ?? {}).length > 50) throw new TypeError('Database view configuration is too large.')
